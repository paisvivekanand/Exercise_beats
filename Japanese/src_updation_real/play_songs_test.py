...
conf_handler.read('/home/ubuntu/headphones/config/conf.ini')
...
def Get_Song_Details(cur_bpm, dictionary):
    ...
    index = dictionary[minimum_val]['index']
    for index in dictionary:
        if (dictionary[index]['Song'] == song):
            dictionary[index]['played'] = 1
    # graph has every 3 rd value n it
    return song, time, bpm, int(index/3)
...
def play_first_song(sound, dictionary):
    ...
    update_in_db_initial(song, 90, 0)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if status == 1:
            if first_song == 1:
                # pulseRateMark = 0
                sound = pygame.mixer.Sound(songsPath + 'Alan_Walker_Faded.wav')
                ...
                if is_end_of_song():
                    ...
                    currsong = song
                    update_in_db(previous_song, currsong, bpm, 0)
                    sound2 = pygame.mixer.Sound(songsPath + song)
                    sound, sound2 = play_song(sound, sound2)
                print("waiting for", pulseRateMark + pulseRateMark / 10)
                time.sleep(1)
                pulseRate = get_pulse_rate()
                print(pulseRateMark)
                print("now", pulseRate)
                # if pulseRateMark < pulseRate:
                # if abs(pulseRateMark - pulseRate) < pulseRateMark / 10:
